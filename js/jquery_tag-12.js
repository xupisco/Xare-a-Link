        ...
        return this.each(function () {
            ...
            function showSuggestions(el, key) {
                ...
                for (i = 0; i < currentTags.length; i++) {
                    if (currentTags[i] != workingTags[i]) {
                ...
                if (currentTag.tag) {
                    ...
                    if (settings.url) {
                        ...
                        for (i = 0; i < userTags.length; i++) {
                            if (userTags[i].indexOf(workingTags[currentTag.position]) === 0) {
                    ...
                    for (i = 0; i < matches.length; i++) {
                        html += '<' + settings.tagWrap + '>' + matches[i] + '</' + settings.tagWrap + '>';
                    }

                    tagMatches.html(html);
					if(html != '') tagMatches.fadeIn();
            ...
            function chooseTag(tag) {
                ...
                if (index == workingTags.length - 1) tag = tag + ' ';

                workingTags[i] = tag;

                tagsElm.val(workingTags.join(' '));
                tagsElm.blur().focus();
                setSelection();
            }
