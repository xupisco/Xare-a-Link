...
function getPR() {
    ...
    if(url != "http://" && url != "0" && url != "") {
		...
		$("#link_title").val("Buscando título...");
		$("#link_desc").val("Buscando descrição...");
	    $.post("ajax/getmeta/", { url: (url) },
	       function(data) {
		   		d = data.split("|||||");
		   		if(d[0] != "error") {
					$("#link_title").val(d[0]);
...
function parseForm() {
	if(!validLink) {
		alert("Problema com a URL. Digite novamente!!!");
		return false;
	}
    $("#n_parents").val(cpath);
    if(!$("#link_title").val()) {
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	<tr>
		...
		<table width="100%" cellpadding="0" cellspacing="0">
			<tr>
				...
				<a href="newlink/rel=<?= $rel; ?>" id="content_name">Novo link</a>
				</td>
			</tr>
		</table>
		</td>
	...
	<tr>
        ...
        <tr><td align="right" valign="top" nowrap="nowrap"><img src="images/common/dot.gif" width="1" height="3" /><br />Endereço (URL): </td><td><input type="text" class="std_input" value="http://" name="link_url" id="link_url" onblur="getPR()" /><div id="quick_pr" style="display: none; font-size: 11px; color: #777777; padding-top: 2px">PageRank: </div><div id="urlerror" style="display: none; font-size: 11px; color: #ff0000; padding-top: 2px">Essa URL já está cadastrada.</div></td><td style="font-size: 11px; color: #777777; padding-left: 10px;" valign="top"><img src="images/common/dot.gif" width="1" height="5" /><br />Não publique links repetidos, faça uma busca antes de incluir.</td></tr>
        <tr><td align="right" width="10%">Título do link: </td><td width="40%"><input type="text" class="std_input" name="link_title" id="link_title" onblur="parseDest(1)" /></td><td style="font-size: 11px; color: #777777; padding-left: 10px;" width="30%">Como o link será listado nas páginas.</td></tr>
        ...
        <? for($i = 0; $i < ($parents_cnt); $i++) { $rs = "lvl_$i"?>
        ...
        <input type="submit" value="Enviar link" style="background-color: #99C145; border: 1px solid #69832F; padding: 6px; color: #ffffff; font-weight: bold" />
        </div>
