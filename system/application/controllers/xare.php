...
class Xare extends Controller {
	...
	function parseURL() {
		...
		foreach($aurl as $dir) {
		    if(strpos($dir, "page") !== false || strpos($dir, "sort") !== false) {
                ...
                $sort = (strpos($dir, "sort") !== false) ? $dir : "sort:related";
            } else {
        ...
        $sorta = explode(":", $sort);
        $sort_1 = (isset($sorta[1])) ? $sorta[1] : "related";

		$parsed['current'] = array_pop($newpath);
		$parsed['url'] = $newpath;
		$parsed['path'] = implode("/", $newpath);
		$parsed['page'] = $page;
		$parsed['sort'] = array($sort_1);
		return $parsed;
	}
	...
	function index() {
		...
		$tags = $this->xare_model->getContentTags($content->id);
    	...
    	$related = $this->xare_model->getContentByParent($content->id, 0, $url['sort'][0], $st1, $sr);
    ...
    function search($t = "s") {
        ...
        if($t == "u") {
        	...
        	if($uid) {
        	   $qs = "u:".$uid;
			   $tags = $this->xare_model->getUserTags($uid);
			   $data['tags'] = $tags;
        	} else {
        ...
        if(!$rss) {        
	        $related = $this->xare_model->getContentByParent(false, 0, $url['sort'][0], $st1, $sr, $qs);
        	...
        	$data['feed'] = $this->xare_model->getContentByParent(false, 0, "name", "ASC", $sr, $qs);
        }
